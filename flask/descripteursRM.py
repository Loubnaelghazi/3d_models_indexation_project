...
def reduce_mesh(vertices, faces, reduction_ratio=0.3):
...
def voxelize(vertices, resolution=64):
    ...
    vertices /= np.ptp(vertices, axis=0)  # Normalisation
...
def extract_face_indices(face):
    """
    Extrait les indices des vertices d'une face à partir d'un dictionnaire.
    """
    if 'vertices' in face:
        # Extraire l'indice des vertices dans la face (on prend uniquement l'indice vertex)
        return [vertex['vertex'] for vertex in face['vertices']]
    return []

def validate_faces(faces):
    """
    Valide les faces en extrayant les indices et en vérifiant que chaque face contient
    trois indices d'entiers.
    """
    valid_faces = []
    for face in faces:
        # Extraire les indices des faces
        indices = extract_face_indices(face)
        if len(indices) == 3 and all(isinstance(idx, int) for idx in indices):
            valid_faces.append(indices)
        else:
            print(f"Élément invalide dans faces: {face}")
    return valid_faces

def process_json_file(json_filepath, collection, resolution=64, size=10, l_max=10, reduction_ratio=0.5):
    ...
    if not isinstance(vertices, list) or not isinstance(faces, list):
        print(f"Erreur : vertices ou faces n'est pas une liste dans {json_filepath}.")
        return

    # Validation et extraction des faces valides
    faces = validate_faces(faces)
    if not faces:
        print(f"Erreur : Aucune face valide trouvée dans {json_filepath}.")
        return
    ...
    for root, dirs, files in os.walk(input_directory):
        for filename in files:
            if filename.endswith('.json'):
                ...
                try:
                    process_json_file(json_filepath, collection, resolution=64, size=10, l_max=10, reduction_ratio=0.5)
